    ...
    try:
        ...
        if member.id in owners:
            if ctx.author.id not in owners:
                return await ctx.send(f"haha no")
            else:
                pass
        if member.id == ctx.guild.owner.id:
            return await ctx.send(f"haha no (owner)")
        if ctx.author.top_role == member.top_role:
            return await ctx.send(f"haha no (same permissions as you)")
        if ctx.author.top_role < member.top_role:
            return await ctx.send(f"haha no (higher permissions than you)")
    except Exception:
        pass
