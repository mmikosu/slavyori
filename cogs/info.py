...
class Information(commands.Cog):
    ...
    async def ping(self, ctx):
        ...
        await message.edit(content=f"Pong! WS: {before_ws}ms  |  REST: {int(ping)}ms")

    @commands.command(aliases=['joinme', 'botinvite'])
    async def invite(self, ctx):
        """ Invite me to your server """
        await ctx.send(f"**{ctx.author.name}**, use this URL to invite me\n<https://discordapp.com/oauth2/authorize?client_id=499391082203840538&scope=bot&permissions=8>")
    ...
    async def about(self, ctx):
        ...
        embed.add_field(name="cmds loaded:", value=len([x.name for x in self.bot.commands]), inline=True)
